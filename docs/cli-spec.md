...
| `--log-format <FORMAT>` | | 로그 파일 파싱 포맷. 사전 정의: `apache`, `apache-combined`, `apache-common`, `nginx`, `syslog`. 사용자 정의: `{field}` 플레이스홀더 패턴 | |
| `--log-error <MODE>` | | 로그 파싱 실패 행 처리: `skip` (건너뛰기) 또는 `raw` (`_raw` 필드로 포함) | `skip` |
| `--parallel <N>` | | 일괄 변환 시 병렬 스레드 수. 숫자 또는 `auto` (CPU 코어 수 자동 감지) | 순차 처리 |
| `--template <STRING>` | | 인라인 Tera 템플릿 문자열 (`-f template` 사용 시). 각 레코드를 템플릿으로 렌더링 | |
| `--template-file <PATH>` | | Tera 템플릿 파일 경로 (`-f template` 사용 시) | |
...
# 로그 파일 파싱 (--log-format)
dkit convert access.log -f json --log-format apache          # Apache Combined 로그 → JSON
dkit convert access.log -f csv --log-format apache-common    # Apache Common 로그 → CSV
dkit convert access.log -f json --log-format nginx           # nginx 로그 → JSON
dkit convert app.log -f json --log-format syslog             # syslog 로그 → JSON
dkit convert app.log -f json --log-format '{timestamp} [{level}] {message}'  # 사용자 정의 패턴
dkit convert app.log -f json --log-format apache --log-error raw  # 파싱 실패 행을 _raw 필드로 포함

# 로그 → 분석 파이프라인
dkit convert access.log -f json --log-format apache | dkit query - '.[] | where status == 404'
dkit stats access.log --log-format apache                    # 로그 통계

# 병렬 일괄 변환 (--parallel)
dkit convert *.json -f csv --outdir ./out/ --parallel 4      # 4 스레드로 병렬 변환
dkit convert *.json -f csv --outdir ./out/ --parallel auto   # CPU 코어 수 자동 감지

# 실행 시간 측정 (--time, 모든 서브커맨드에서 사용 가능)
dkit convert data.json -f csv --time                         # 변환 시간 측정
dkit query data.json '.users[]' --time                       # 쿼리 시간 측정
dkit stats data.csv --time                                   # 통계 시간 측정

# 템플릿 출력 (--template, feature flag: template)
dkit convert users.json -f template --template '{{ name }} <{{ email }}>'       # 인라인 템플릿
dkit convert data.json -f template --template-file report.hbs                  # 파일 기반 템플릿
dkit convert data.json -f template --template '{{ name | upper }}'             # 필터 사용 (upper, lower, default)
dkit convert stats.json -f template --template '## {{ title }}\n- Count: {{ count }}'  # Markdown 생성
```
...
| `-o <FILE>` | 출력 파일 |
| `--explain` | 쿼리 실행 계획만 출력하고 실행하지 않음 (stderr 출력) |
| `--log-format <FORMAT>` | 로그 파일 입력 시 파싱 포맷 지정 |
| `--log-error <MODE>` | 로그 파싱 실패 행 처리 (`skip` / `raw`) |
...
# 실행 계획 확인 (--explain)
dkit query data.json '.[] | where age > 20 | select name, age | sort age desc | limit 5' --explain
# → Execution Plan:
# →   1. Scan: data.json
# →   2. Navigate: .[]
# →   3. Filter: age > 20
...
두 데이터 파일을 공통 키로 결합 (JOIN).

### Usage

```bash
dkit join <LEFT> <RIGHT> --on <KEY> [OPTIONS]
```
...
| Option | Short | Description | Default |
|--------|-------|-------------|---------|
| `--on <KEY>` | | 결합 키. `field` (양쪽 동일) 또는 `left_field=right_field` | 필수 |
| `--type <TYPE>` | | JOIN 타입: `inner`, `left`, `right`, `full` | `inner` |
| `--format <FORMAT>` | `-f` | 출력 포맷 (json, csv, yaml 등) | |
| `--output <FILE>` | `-o` | 출력 파일 경로 | stdout |
| `--pretty` | | 포맷팅된 출력 | false |
| `--compact` | | 한 줄 출력 (JSON) | false |
| `--delimiter <CHAR>` | | CSV 구분자 | `,` |
| `--no-header` | | CSV 헤더 없음 | false |
| `--encoding <ENCODING>` | | 입력 파일 인코딩 | UTF-8 |
| `--detect-encoding` | | 인코딩 자동 감지 | false |
| `--sheet <SHEET>` | | Excel 시트 이름 또는 인덱스 | 첫 번째 시트 |
| `--header-row <N>` | | Excel 헤더 행 번호 (1-based) | 1 |
...
| `left` | 왼쪽 파일의 모든 행 + 매칭되는 오른쪽 행 |
| `right` | 오른쪽 파일의 모든 행 + 매칭되는 왼쪽 행 |
| `full` | 양쪽 모든 행 (매칭 안 되면 null) |
...
# FULL OUTER JOIN + Pretty 출력
dkit join a.json b.json --on id --type full -f json --pretty

# 파일로 출력
dkit join users.json orders.json --on user_id -f csv -o joined.csv
```
...
| Option | Short | Description | Default |
|--------|-------|-------------|---------|
| `--output-format <FORMAT>` | `-O` / `-f` | 출력 포맷: `table`, `json`, `yaml`, `md` | `table` |
| `--detailed` | | 상세 프로파일링 (히스토그램, 이상치, 필드별 상세) | false |
...
| `--table <TABLE>` | | SQLite 테이블 이름 | 첫 번째 테이블 |
| `--sql <SQL>` | | SQLite 커스텀 SQL 쿼리 | |
| `--log-format <FORMAT>` | | 로그 파일 파싱 포맷 | |
| `--log-error <MODE>` | | 로그 파싱 실패 행 처리 | `skip` |

### 프로파일링 항목

**필드별 기본 정보:**
- 데이터 타입 (추론): `str`, `int`, `float`, `bool`
- Null/빈값 비율 (%)
- 유니크 값 수 (cardinality)
- 최빈값 (top value)
- 패턴 감지 (이메일, enum, 숫자 범위 등)
...
# YAML 출력
dkit profile data.csv --output-format yaml

# Markdown 출력
dkit profile data.csv --output-format md

# 상세 프로파일링
dkit profile data.csv --detailed

# JSON 입력
dkit profile data.json

# stdin 입력
cat data.csv | dkit profile - --from csv

# 로그 파일 프로파일링
dkit profile access.log --log-format apache
```
...
| `--time` | 실행 시간 측정 (parse, transform, write 단계별 시간 + 총 시간, stderr 출력) |
```
